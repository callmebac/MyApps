package csu.example.lightcontrol;
...
import csu.example.netConn.dataSave;
import csu.example.netConn.netMsg;
import csu.example.netConn.netThread;
import csu.example.netConn.packageData;

import java.io.IOException;

public class LightControlActivity extends Activity implements SensorEventListener {
    ...
    protected void onResume() {
        ...
        sensorManager.registerListener(LightControlActivity.this, sensor, SensorManager.SENSOR_DELAY_NORMAL);
    }
    ...
    protected void onPause() {
        super.onPause();
        sensorManager.unregisterListener(LightControlActivity.this);
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        //两次检测的时间间隔
        long timeInterval = currentTime - lastTriggerTime;

        EditText macAddr = (EditText) findViewById(R.id.macAddress);

        float lightValue = event.values[0];

        //if(sensorType == Sensor.TYPE_LIGHT && netConn.isConnect()){
...
/*                AlertDialog.Builder builder = new AlertDialog.Builder(LightControlActivity.this);
                ...
                builder.setMessage("timeInterval: " + timeInterval + "; lightValue: " + lightValue);
                builder.setPositiveButton("Yes", null);
                builder.show();*/
                vibrator.vibrate(300);
                packageData pData = new packageData(macAddr.getText().toString());
                netConn.sendMsg(pData.winOpenData());
            }else if (lightValue > 640){
                //更新lastShakeTime为当前时间
                lastTriggerTime = currentTime;
/*                AlertDialog.Builder builder = new AlertDialog.Builder(LightControlActivity.this);
                builder.setTitle("Test!");
